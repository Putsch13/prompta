...
### Tâches
- [ ] Copier `delivery/supabase/migrations/0004_admin_agents.sql` dans `supabase/migrations/`.
- [ ] Copier `delivery/supabase/migrations/0005_sandbox.sql` dans `supabase/migrations/`.
- [ ] Exécuter les DEUX migrations dans Supabase → SQL Editor (0004 puis 0005).
...
### Tâches
- [ ] Ajouter dans `.env.local` ET dans Render → Environment :
  ```
  ANTHROPIC_API_KEY=sk-ant-...
  CRON_SECRET=<chaîne aléatoire longue, ex: openssl rand -hex 32>
  ```
- [ ] Mettre à jour `lib/env.ts` : ajouter `ANTHROPIC_API_KEY` et `CRON_SECRET` au `envSchema` (server-side uniquement, PAS dans `clientSchema`).
- [ ] Copier `delivery/lib/agents/` complet dans `lib/agents/` : `budget.ts`, `anthropic.ts`, `runner.ts`, `types.ts`.
- [ ] Copier `delivery/lib/admin/guard.ts` dans `lib/admin/`.
...
### Tâches
- [ ] Copier tout `delivery/agents/` dans `agents/` à la racine : `index.ts` + les 7 fichiers.
- [ ] Vérifier le `tsconfig.json` : le path `@/*` doit couvrir la racine pour que `@/agents` résolve. Si besoin, ajouter `"@/agents": ["./agents/index.ts"]` ou garder `@/*` → `./*`.
- [ ] Lire chaque agent et adapter les `CATEGORIES`, profils-cibles, etc. à ta réalité.
...
### Tâches
- [ ] Copier `delivery/app/api/cron/tick/route.ts` → `app/api/cron/tick/route.ts`.
- [ ] Copier `delivery/app/api/agents/run/route.ts` → `app/api/agents/run/route.ts`.
- [ ] Copier `delivery/app/api/agents/approve/route.ts` → `app/api/agents/approve/route.ts`.
- [ ] Copier `delivery/app/api/agents/sandbox/route.ts` → `app/api/agents/sandbox/route.ts`.
- [ ] Vérifier que `lib/slug.ts` exporte `uniqueSlug` (utilisé par `approve`).
- [ ] Mettre à jour `middleware.ts` : ajouter `/admin` à `PROTECTED_ROUTES`.
...
### Tâches
- [ ] Copier `delivery/app/admin/layout.tsx` → `app/admin/layout.tsx`.
- [ ] Copier `delivery/app/admin/page.tsx` → `app/admin/page.tsx` (dashboard KPI).
- [ ] Copier `delivery/app/admin/agents/page.tsx` → `app/admin/agents/page.tsx`.
- [ ] Copier `delivery/app/admin/agents/AgentsControlPanel.tsx` → `app/admin/agents/AgentsControlPanel.tsx`.
- [ ] Vérifier que `createServerClient` est bien exporté par `lib/supabase/server.ts` (sinon adapter l'import dans `guard.ts`).
- [ ] Ajouter un lien discret vers `/admin` dans `components/Header.tsx`, visible **uniquement si l'utilisateur est admin**.
...
### Tâches
- [ ] Installer `tsx` en dev : `npm i -D tsx`.
- [ ] Copier `delivery/scripts/seed-personas.ts` → `scripts/seed-personas.ts`.
