    ...
    <div className="space-y-4 rounded-xl border border-line bg-card2 p-4">
      ...
      {summary.sharedProvided.length > 0 && (
        <div className="border-t border-line pt-3">
          <p className="text-xs font-semibold text-violet-300">Fourni par vous (partagé 🌐)</p>
          <ul className="mt-1 space-y-1">
            {summary.sharedProvided.map((item) => (
              <li
                key={item.id}
                className="rounded border border-violet-400/30 bg-violet-400/10 px-2 py-1 text-[10px] text-violet-200"
              >
                {item.label}
